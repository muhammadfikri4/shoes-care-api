...
export const SendTransactionNotificationEmail = async (payload: {
  ...
  // 2) Pakai CID pada <img>, fallback ke <pre> bila gagal generate
  const qrBlock = qrPng
    ? `<img src="cid:qr-pickup" alt="QR Pickup" style="width:200px;height:200px" />`
    : `<pre style="background:#f5f5f5;padding:8px;border-radius:8px">${qrData}</pre>`;
